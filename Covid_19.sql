...
SELECT continent,location,((SUM(CAST(new_cases AS FLOAT))-SUM(CAST(new_deaths AS FLOAT)))/SUM(CAST(new_cases AS INT)))*100 as recovery_rate
FROM CovidDeaths
WHERE continent IS NOT NULL
...
DROP TABLE IF EXISTS RecoveryRate;
CREATE table RecoveryRate
(Continent nvarchar(50),
Country nvarchar(50),
Recovery_rate float);

INSERT INTO RecoveryRate
SELECT continent,location,((SUM(CAST(new_cases AS FLOAT))-SUM(CAST(new_deaths AS FLOAT)))/SUM(CAST(new_cases AS INT)))*100 as recovery_rate
FROM CovidDeaths
WHERE continent IS NOT NULL
GROUP BY location,continent
...
SELECT * FROM RecoveryRate
ORDER BY Continent


-- death rate by country

SELECT continent,location,(SUM(CAST(new_deaths AS FLOAT))/SUM(CAST(new_cases AS FLOAT)))*100 as Death_rate
FROM CovidDeaths
WHERE continent IS NOT NULL
GROUP BY location,continent
ORDER BY Death_rate;
